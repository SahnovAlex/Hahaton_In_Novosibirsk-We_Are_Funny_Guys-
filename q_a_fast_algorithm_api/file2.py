...
def handle_user_input(user_input, current_context, history):
    history.append(("user", user_input))
    
    if user_input.strip().lower() == 'exit':
        return {"response": "exit"}, current_context, history
    ...
    if current_context and user_input.isdigit():
        ...
        if 1 <= num <= len(current_context['options']):
            ...
            response_text = f"Ответ: {answers[idx]}\nИсточник: {sources[idx]}"
            history.append(("system", f"answer_idx={idx}"))
            current_context = None
            return {"response": response_text}, current_context, history

    # Поиск лучших совпадений
    scored = find_best_matches(user_input)
    if not scored:
        current_context = None
        return {"response": "Извините, я не нашёл подходящего ответа в базе знаний."}, current_context, history
    ...
    if has_domain_overlap and best_score >= DIRECT_ANSWER_MIN:
        current_context = None
        return {"response": f"Ответ: {answers[best_idx]}\nИсточник: {sources[best_idx]}"}, current_context, history
